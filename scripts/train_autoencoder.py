...
from piwm_diffusion.autoencoder import PiwmConvVAE, piwm_vae_loss
    ...
    for batch in tqdm(loader, desc="train" if train else "eval", leave=False):
        ...
        with torch.set_grad_enabled(train):
            ...
            losses = piwm_vae_loss(
                ...
                kl_on_physical=args.kl_on_physical,
            )
            crop_loss = (
...
def main():
    ...
    parser.add_argument("--kl_on_physical", action="store_true")
    ...
    if len(args.state_indices) > args.latent_dim:
        raise ValueError("state_indices cannot exceed latent_dim")
    ...
    model = PiwmConvVAE(latent_dim=args.latent_dim).to(device)
    ...
    print("device:", device)
    print("state:", list(zip(state_spec.indices, state_spec.names)))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with torch.no_grad():
            # Random validation examples are less biased than the first trajectory.
            ...
            save_reconstruction_grid(
                ...
                recon,
                epoch_grid_path,
                suptitle=f"Autoencoder reconstruction - epoch {epoch}",
            )
        ...
        if test_metrics["loss"] < best_loss:
            ...
            save_reconstruction_grid(
                ...
                suptitle=f"Best autoencoder reconstruction - epoch {epoch}",
            )
            if args.crop_weight > 0.0:
                save_reconstruction_grid_boxed(
                    ...
                    crop_size=args.crop_size,
                    suptitle=f"Best autoencoder reconstruction with cyan crop boxes - epoch {epoch}",
                )
            print("saved best")
        else:
            stale += 1
