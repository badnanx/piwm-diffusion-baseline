#!/usr/bin/env python
"""
Evaluate the physical encoder visually.

Produces two outputs:
  scatter.png  -- pred vs true scatter plot for each state variable
...
def collect_predictions(autoencoder, physical_model, loader, state_indices, device):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            pred_state, _ = physical_model(mu)
            all_pred.append(pred_state.cpu())
            all_true.append(states[:, state_indices].cpu())
            all_images.append(images.cpu())
    return (
        torch.cat(all_pred, dim=0).numpy(),
...
def plot_scatter(pred, true, state_names, output_path):
    ...
    fig.suptitle("Physical encoder: predicted vs true state variables", fontsize=13)
    fig.tight_layout()
...
def plot_overlay(images, pred_states, true_states, n_show, output_path):
    ...
    fig.suptitle("Physical encoder position overlay (green=true, red=predicted)", fontsize=11)
...
def main():
    ...
    parser.add_argument("--output_dir", default="outputs/physical_eval")
    ...
    pred, true, images = collect_predictions(
        autoencoder, physical_model, loader, state_idx_tensor, device
    )

    print(f"collected {len(pred)} frames")
    for i, name in enumerate(state_names):
        rmse = float(np.sqrt(np.mean((pred[:, i] - true[:, i]) ** 2)))
        print(f"  {name}: RMSE={rmse:.4f}  true=[{true[:,i].min():.2f}, {true[:,i].max():.2f}]  pred=[{pred[:,i].min():.2f}, {pred[:,i].max():.2f}]")
    ...
    if has_xy:
        # reindex pred/true so index 0=x, 1=y within our state slice
